...
//  Portfolioish
//
//  Created by Deirdre Saoirse Moen on 2/20/22.
//
...
enum RequestState {
	case loading				//	started request, but no answer yet
	case loaded(SKProduct)		//	successful response
	case failed(Error?)			//	unsuccessful response
	case purchased				//	we bought it!
	case deferred				//	need to wait for permission
}
...
class UnlockManager: NSObject, ObservableObject,
	...
	init(inMemory: Bool = false, persistence: Persistence) {
		...
		//	return if we're already unlockified
		if persistence.fullVersionUnlocked == true { return }
		...
		DispatchQueue.main.async { [self] in
			for transaction in transactions {
				...
				case .purchased, .restored:
					persistence.fullVersionUnlocked = true
